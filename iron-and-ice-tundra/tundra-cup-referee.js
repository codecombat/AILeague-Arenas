...
const GUARD_BASE_DAMAGE_COEF = 0.25;
...
const WAVES = [
    ...
    [
        ['rider', 'rider', 'braweler'], ['rider', 'rider', 'warlock'], ['rider', 'rider', 'rider'], 
        ],
    [
...
({ 
    ...
    setupHeroes() {
        ...
        for (let h of this.heroes) {
            ...
            h.takeDamage = (damage, who) => {
                ...
                // }
                if (who.type == 'guard') {
                    damage = damage * GUARD_BASE_DAMAGE_COEF;
                }
    ...
    onFirstFrame(){
        ...
        // this.setTimeout(() => {
        let guardRed1 = this.hero.spawnGuard('S');
        let guardRed2 = this.hero.spawnGuard('T');
        let guardBlue1 = this.hero2.spawnGuard('S');
        let guardBlue2 = this.hero2.spawnGuard('T');
        // }, this.world.dt);
        this.setTimeout(() => {
            this.hero.triggerSpawnGuardEvent(guardRed1, 'S');
            this.hero.triggerSpawnGuardEvent(guardRed2, 'T');
            this.hero2.triggerSpawnGuardEvent(guardBlue1, 'S');
            this.hero2.triggerSpawnGuardEvent(guardBlue2, 'T');
        }, this.world.dt);
    ...
    setupMonster(monster, type) {
        ...
        monster.mana = params.cost;
        monster.mass = Math.pow(params.maxHealth / 10, 0.33) * 10;
        monster.appendMethod('takeDamage', function(damage, attacker) {
            if (attacker.isGuard) {
    ...
    monster_chooseAction() {
        ...
        if (this.aggro && !this.aggro.dead && this.aggro.exists) {
