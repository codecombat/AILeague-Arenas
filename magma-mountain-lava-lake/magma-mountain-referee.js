...
({
    ...
    findItemPos() {
        ...
        let posM = new Vector(this.maxWidth - pos.x, this.maxHeight - pos.y);
    ...
    heroOnHasMoved(h, prevPos, newPos) {
        let dxy;
        if (h.pos.x < 0 || h.pos.x >= this.mapWidth * this.step || h.pos.y < 0 || h.pos.y >= this.mapHeight * this.step) {
            if (h.targetPos) {
            ...
            if (!h.jumpCell || (h.jumpCell.row == h.prevCell.row && h.jumpCell.col == h.prevCell.col)) {
                ...
                if (h.lastDirection == 'up') {
                    ...
                    for (let r = fromRow; r < toRow; r++) {
                        this.createLavaCell(r % this.maxRows, h.currentCell.col, lavaLifespan);
                    }
                }
                    ...
                    if (h.wasWrapped) {
                        ...
                        for (let r = fromRow; r > toRow; r--) {
                            this.createLavaCell((r + this.maxRows) % this.maxRows, h.currentCell.col, lavaLifespan);
                        }
                    }
                }
                    ...
                    if (h.wasWrapped) {
                        ...
                        for (let c = fromCol; c < toCol; c++) {
                            this.createLavaCell(h.currentCell.row, c % this.maxCols, lavaLifespan);
                        }
                    }
                }
                    ...
                    if (h.wasWrapped) {
                        ...
                        for (let c = fromCol; c > toCol; c--) {
                            this.createLavaCell(h.currentCell.row, (c + this.maxCols) % this.maxCols, lavaLifespan);
                        }
                    }
                }
            }
    ...
    createLavaCell(row, col, lifespan) {
        if (row < 0 || row >= this.maxRows || col < 0 || col >= this.maxCols) return;
    ...
    fireballUpdate(fireball) {
        if (fireball.pos.x < 0 || fireball.pos.x >= this.maxWidth ||
                fireball.pos.y < 0 || fireball.pos.y >= this.maxHeight) {
        ...
        if (fireball.prevCell.row != row || fireball.prevCell.col != col) {            
            ...
            if (fireball.lastDirection == 'up') {
                for (let r = fromRow; r < toRow; r++) {
                    for (let i = -1; i <= 1; i++) {
                        this.createLavaCell(r, fireball.currentCell.col + i, fireball.power);
                    }
                }
            }
                ...
                for (let c = fromCol; c < toCol; c++) {
                    for (let i = -1; i <= 1; i++) {
                        this.createLavaCell(fireball.currentCell.row + i, c, fireball.power);
                    }
                }
            }
    ...
    detectLavaType(r, c) {
        if (this.lavaMap[r][c] <= 0) {
            return null;
        }
        let code = "lava";
        
        if (r == this.maxRows - 1 || this.lavaMap[r + 1][c] > 0) {
            code += '0';
        }
        else {
            code += '1';
        ...
        if (c == this.maxCols - 1 || this.lavaMap[r][c + 1] > 0) {
            code += '0';
        }
        else {
            code += '1';
        ...
        for (let y = 1; y < this.maxRows - 1; y++) {
            for (let x = 1; x < this.maxCols - 1; x++) {
                if (this.lavaMap[y][x] > 0) {
                    ...
                    if (this.lavaMap[y][x] <= 0) {
                        this.lavaMap[y][x] = 0;
                        // this.lavas.filter((l) => l.row == y && l.col == x).forEach((l) => l.setExists(false));
                    }
                }
            }
        }
        // this.lavas = this.lavas.filter((l) => l.exists);
